...
class CustomerBase(BaseModel):
    ...
    mobile_number:str = constr(strip_whitespace=True, pattern=r"^\d{10}$")
    gender: GenderEnum
    aadharcard: str
    pancard: str
    branch_id: int
...
class UpdateCustomerBase(BaseModel):
    ...
    mobile_number:Optional[str] = constr(strip_whitespace=True, pattern=r"^\d{10}$")
...
class UserData(BaseModel):
    ...
    mobile_number:int  = constr(strip_whitespace=True, pattern=r"^\d{10}$") # Ensuring exactly
    manager_id:int
